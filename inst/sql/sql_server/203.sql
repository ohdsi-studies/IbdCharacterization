...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4135421,4261660)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4135421,4261660)
  and c.invalid_reason is null
...
WHERE (C.value_as_number >= 18.5000 and C.value_as_number <= 25.0000)
...
WHERE not (C.value_as_number >= 18.5000 and C.value_as_number <= 25.0000)
-- End Measurement Criteria
